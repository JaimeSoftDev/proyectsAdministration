...
if (isset($_REQUEST["evento"])) {
    ...
    switch ($_REQUEST["evento"]) {
        ...
        case "filtrar":
            ...
            if (!isset($_REQUEST["misProyectos"]))
                $projects = $controlador->buscar($campo, $metodo, $texto, comprobarSiEsBorrable: true);
            else
                $projects = $controlador->buscarPorUsuarioSesion($_SESSION["usuario"], $campo, $metodo, $texto, comprobarSiEsBorrable: true); //solo añadimos esto
            break;
        case "borrar":
...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    <div
        ...
        <h1 class="h3">Buscar Proyecto</h1><a class="btn btn-dark" href="index.php?tabla=project&accion=crear"><i
                class="fa fa-plus"></i> Nuevo Proyecto</a>
    </div>
    <div id="contenido">
            ...
            <form action="index.php?accion=buscar&tabla=project&evento=filtrar" method="POST">
                <div class="form-group">
                    <select class="form-select" name="campo" id="campo">
                        <option value="projects.id" <?= $campo == "projects.id" ? "selected" : "" ?>>ID</option>
                        <option value="projects.name" <?= $campo == "projects.name" ? "selected" : "" ?>>Nombre Proyecto
                        </option>
                    ...
                    <input class="form-check-input" type="checkbox" value="checked" name="misProyectos"
                        id="misProyectos" <?= $_REQUEST["misProyectos"] ?? "" ?>>
                    <label class="form-check-label" for="misProyectos">
                        Mis Proyectos
                    </label>
                    <DIV>
                        <button type="submit" class="btn btn-success" name="Filtrar">Buscar</button>
                        <a href="index.php?accion=buscar&tabla=project&evento=todos" class="btn btn-info" name="Todos"
                    ...
                    <tbody>
                        <?php foreach ($projects as $project):
                            ...
                            <tr>
                                ...
                                <td>
                                    <?= "{$project->user_id} - {$project->usuario_user} {$project->name_user} " ?>
                                </td>
                                <td>
                                    <?= "$project->client_id - {$project->contact_name_client} {$project->company_name_client} " ?>
                                </td>
                                <td><a class="btn btn-danger <?= $disable ?>" href="<?= $rutaBorrar ?>"><i class="fa fa-trash"></i>
                                        Borrar</a></td>
                                <td><a class="btn btn-warning" href="index.php?tabla=project&accion=ver&id=<?= $id ?>"><i
